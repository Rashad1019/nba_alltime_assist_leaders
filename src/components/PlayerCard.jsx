...
const PlayerCard = ({ player, index }) => {
    ...
    return (
        ...
        >
            ...
            <div style={{ display: 'flex', flexDirection: 'row', minHeight: '240px' }}>
                ...
                >
                    <div className="absolute top-3 left-3">
                        <span className="text-3xl font-black text-gold">#{player.rank}</span>
                    </div>

                    <img
                        src={player.image}
                        alt={player.name}
                        className="w-24 h-24 object-cover rounded-full border-4 border-[#ffd700]/40 shadow-xl"
                        onError={(e) => { e.target.style.display = 'none'; }}
                    />

                    <h3 className="text-lg font-bold text-white mt-3 text-center">{player.name}</h3>
                ...
                >
                    ...
                    {player.active && player.currentSeasonApg && (
                        <div className="mt-3 bg-[#ff6b35]/20 border border-[#ff6b35]/30 rounded-lg p-2 text-center">
                            <span className="text-orange text-sm">🔥 Current: </span>
                            <span className="text-gold font-bold">{player.currentSeasonApg} APG</span>
                        </div>
                    )}
                </div>
